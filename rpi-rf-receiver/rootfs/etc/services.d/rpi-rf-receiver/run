...
if ! bashio::services.available "mqtt" && ! bashio::config.exists 'mqtt.host'; then
    ...
    if ! bashio::config.exists 'mqtt.host' || ! bashio::config.exists 'mqtt.port'; then
        bashio::log.info "MQTT host/port settings not configured, trying to auto-discovering ..."
...
VALID_CODES="$(bashio::config 'valid_codes')"
VALID_CODES=$(echo -n $VALID_CODES | tr '\n' ' ')

bashio::log.info "Launching command: python3 /rpi-rf_receive.py --log-level $LOG_LEVEL --valid-codes $VALID_CODES -g $GPIO --mqtt-host $MQTT_HOST --mqtt-port $MQTT_PORT --mqtt-user $MQTT_USER --mqtt-password XXXX"
python3 /usr/bin/rpi-rf-receive.py --log-level $LOG_LEVEL --valid-codes $VALID_CODES -g $GPIO --mqtt-host $MQTT_HOST --mqtt-port $MQTT_PORT --mqtt-user $MQTT_USER --mqtt-password $MQTT_PASSWORD
